...
gb_fit |>
  ...
  ggplot(aes(x = learn_rate, y = mean, color = factor(tree_depth), group = factor(tree_depth))) +
...
final_gb_fit <- extract_workflow(final_gb_fit)

final_gb_fit |>
  extract_fit_parsnip() |>
  vip()

saveRDS(final_gb_fit, "final_fit_gb.rds")
```