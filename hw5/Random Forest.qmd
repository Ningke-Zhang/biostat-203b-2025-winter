...
rf_fit |>
  ...
  scale_x_continuous(breaks = seq(min(rf_grid$trees), max(rf_grid$trees), by = 50)) +  # 让 x 轴更清晰
  theme_minimal()
```
6. Model evaluation
```{r}
...
final_rf_fit <- extract_workflow(final_rf_fit)

final_rf_fit |>
  extract_fit_parsnip() |>
  vip()

saveRDS(final_rf_fit, "final_fit_rf.rds")
```