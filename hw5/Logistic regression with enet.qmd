...
logit_fit |>
  ...
  print(width = Inf) |> 
  ggplot(mapping = aes(x = penalty, y = mean, color = factor(mixture), group = factor(mixture))) +  
  geom_point() +
  geom_line() +
...
final_logit_fit <- extract_workflow(final_logit_fit)

final_logit_fit |>
  extract_fit_parsnip() |>
  vip()

saveRDS(final_logit_fit, "final_fit_logistic.rds")
```