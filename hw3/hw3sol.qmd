...
race <- if("race" %in% colnames(admissions)) tolower(admissions$race) else "unknown"
...
ggplot() +
  geom_segment(data = transfers, 
               ...
               size = 3) +
  geom_point(data = labevents, aes(x = charttime, y = "Lab"), shape = 3, size = 3) +
  geom_point(data = procedures_icd, aes(x = chartdate, y = "Procedure", shape = long_title), size = 4) +
  labs(
    title = paste("Patient", sid, ",", patients$gender, ",", patients$anchor_age, "years old,", race),
    subtitle = paste(diagnoses_icd$long_title, collapse = "\\n"),
    x = "Calendar Time",
    y = NULL
  ) +
  scale_color_manual(name = "Care Unit", values = scales::hue_pal()(length(unique_careunits))) +
