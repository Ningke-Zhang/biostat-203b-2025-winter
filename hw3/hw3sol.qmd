...
# Rename columns based on d_items_tble mapping
chartevents_tble <- chartevents_tble |>
  rename_with(~ str_to_lower(d_items_tble$label[match(.x, d_items_tble$itemid)]), 
              .cols = intersect(names(chartevents_tble), d_items_tble$itemid))  # Ensure only valid column names are matched

# Fix relocate() by explicitly defining column order
col_order <- c("subject_id", "stay_id", setdiff(names(chartevents_tble), c("subject_id", "stay_id")))
chartevents_tble <- chartevents_tble |> 
...
for (var in lab_vars) {
  p <- ggplot(mimic_icu_cohort, aes_string(x = var, y = "los")) +
    ...
    labs(x = var, y = "Length of ICU Stay (LOS)", title = paste("ICU Stay vs", var)) +
    theme_minimal()
  
  plots[[var]] <- p  # Store plot in list
}

mimic_icu_cohort |> 
  pivot_longer(cols = all_of(lab_vars), names_to = "Lab Test", values_to = "Value") |> 
...
mimic_icu_cohort |> 
  pivot_longer(cols = all_of(vital_vars), names_to = "Vital Sign", values_to = "Value") |> 
  ggplot(aes(x = Value, y = los)) +
  geom_point(alpha = 0.3, color = "blue") +
  geom_smooth(method = "lm", color = "red", se = FALSE) +
